...
def load_stopwords(path):
    """
    Load stopwords from path and add to default STOPWORDS list
    return: a set of stopwords
    """
    stpwrd = nltk.corpus.stopwords.words('english')
    new_words = list(pd.read_csv(path)['Word'])
    stpwrd.extend(new_words)
    return stpwrd
...
def draw_word_cloud_with_stopword(text_series, stopwords, max_word):
    """
    Draw wordcloud for textual reviews gathered
    Arguments:
    text_series: ngram textual reviews, pandas series
    ...
    wordcloud = WordCloud(max_words=max_word, stopwords=stopwords, background_color="white", random_state=1).generate(text_series.str.cat(sep=' '))
